...
A minimal single-player Ludo web scaffold built with React, TypeScript, and Vite. It now includes a basic, locally run rules engine (turn order, legal move generation, capturing, and safe tiles) plus a Home page for configuring human/AI players and a Game page with a canvas-based "Board Placeholder" ready for future board rendering.
...
- `src/pages/Game.tsx` – basic turn loop UI (roll dice, pick a legal move list, apply move) with a canvas placeholder for the board
...
- Next steps: render the real board on canvas and add AI move selection.
